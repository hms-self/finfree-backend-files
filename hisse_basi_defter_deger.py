...
with open('sanaiyes.csv', 'r') as p_file:
    with open('hisse_basi_defter_deger.csv', 'w') as n_file:
                ...
                if j==2020:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        hisse_basi_defter_deger = round(((float(
                                            lists1[247][m].replace('.', '').replace(',', '.')[:-3]) / (float(
                                            lists1[249][m].replace('.', '').replace(',', '.')[:-3])))), 2)

                                    except ZeroDivisionError:
                                        hisse_basi_defter_deger = 0
                                    lists[m].extend([str(hisse_basi_defter_deger).replace('.',',')])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    hisse_basi_defter_deger= round(((float(lists1[247][m].replace('.','').replace(',', '.')[:-3]) / (float(lists1[249][m].replace('.','').replace(',', '.')[:-3])))), 2)
