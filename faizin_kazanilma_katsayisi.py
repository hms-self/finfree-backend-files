...
with open('sanaiyes.csv', 'r') as p_file:
    with open('faizin_kazanilma_katsayisi.csv', 'w') as n_file:
                ...
                if j==2020:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        faizin_kazanilma_katsayisi = round(((float(
                                            lists1[354][m].replace('.', '').replace(',', '.')[:-3])) / (float(
                                            lists1[367][m].replace('.', '').replace(',', '.')[:-3]))), 2)

                                    except ZeroDivisionError:
                                        faizin_kazanilma_katsayisi = 0
                                    lists[m].extend([str(faizin_kazanilma_katsayisi).replace('.',',')])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    faizin_kazanilma_katsayisi = round(((float(lists1[354][m].replace('.', '').replace(',', '.')[:-3]))/(float(lists1[367][m].replace('.', '').replace(',', '.')[:-3]))), 2)
