...
with open('sanaiyes.csv', 'r') as p_file:
    with open('ozsermaye_karllilik_orani_son.csv', 'w') as n_file:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        ozsermaye_karllilik_orani = round(((float(lists1[375][m].replace('.','').replace(',', '.')[:-3]) / (float(lists1[247][m].replace('.','').replace(',', '.')[:-3])))*100), 2)

                                    except ZeroDivisionError:
                                        ozsermaye_karllilik_orani = 0
                                    lists[m].extend([str('%')+str(ozsermaye_karllilik_orani)])
