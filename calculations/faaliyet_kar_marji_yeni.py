...
with open('sanaiyes.csv', 'r') as p_file:
    with open('faaliyet_kar_marji_yeni.csv', 'w') as n_file:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        faaliyet_kar_marji = round(((
                                                float(lists1[354][m].replace('.', '').replace(',', '.')[:-3]) / (
                                    ...
                                    lists[m].extend(['%'+str(faaliyet_kar_marji)])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    faaliyet_kar_marji = round(((
                                             float(lists1[354][m].replace('.', '').replace(',', '.')[:-3]) / (float(lists1[315][m].replace('.', '').replace(',', '.')[:-3])))) * 100, 2)  #esas faaliyet kar/hasılat yüzdesi
