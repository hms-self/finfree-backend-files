...
with open('sanaiyes.csv', 'r') as p_file:
    with open('temettu_dagitma_orani1.csv', 'w') as n_file:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        temettu_dagitma_orani = round(((float(lists1[602][m].replace('.','').replace(',', '.')[:-3]) / (float(lists1[615][m].replace('.','').replace(',', '.')[:-3]))))*100, 2)

                                    except ZeroDivisionError:
                                        temettu_dagitma_orani = 0
                                    lists[m].extend([str('%')+str(temettu_dagitma_orani)])
