...
with open('sanaiyes.csv', 'r') as p_file:
    with open('uzunvadeborc_kaynak_oranı.csv', 'w') as n_file:
                ...
                if j==2020:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        uzunvadeborc_kaynak_oranı = round(((float(
                                            lists1[195][m].replace('.', '').replace(',', '.')[:-3]) / (float(
                                            lists1[306][m].replace('.', '').replace(',', '.')[:-3]))) * 100), 2)

                                    except ZeroDivisionError:
                                        uzunvadeborc_kaynak_oranı = 0
                                    lists[m].extend([str('%') + str(uzunvadeborc_kaynak_oranı)])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    uzunvadeborc_kaynak_oranı = round(((float(lists1[195][m].replace('.','').replace(',', '.')[:-3]) / (float(lists1[306][m].replace('.','').replace(',', '.')[:-3])))*100), 2)
