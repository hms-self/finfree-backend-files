...
with open('sanaiyes.csv', 'r') as p_file:
    with open('net_kar_marji.csv', 'w') as n_file:
                            ...
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        ...
                                        net_kar_marji = 0
                                    lists[m].extend(['%'+str(net_kar_marji)])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    net_kar_marji = round(((
                                             float(lists1[375][m].replace('.', '').replace(',', '.')[:-3]) / (float(lists1[315][m].replace('.', '').replace(',', '.')[:-3])))) * 100, 2)  #esas faaliyet kar/hasılat yüzdesi
