...
with open('sanaiyes.csv', 'r') as p_file:
    with open('kaldirac_orani.csv', 'w') as n_file:
                ...
                if j==2020:
                            ...
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        ...
                                        kaldirac_orani = 0
                                    lists[m].extend([str(kaldirac_orani).replace('.', ',')])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    kaldirac_orani = round(((float(lists1[133][m].replace('.','').replace(',', '.')[:-3]))+(float(lists1[195][m].replace('.','').replace(',', '.')[:-3]))) / (float(lists1[132][m].replace('.','').replace(',', '.')[:-3])), 2)
