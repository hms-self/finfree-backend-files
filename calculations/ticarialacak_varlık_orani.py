...
with open('sanaiyes.csv', 'r') as p_file:
    with open('ticarialacak_varlik_orani.csv', 'w') as n_file:
                ...
                if j==2020:
                        ...
                        for n in range(3,9,3):
                            with open(str(b) + ' sanai ' + str(n) + '.csv', 'r', encoding='utf-8-sig') as reader_file1:
                                ...
                                for m in range(1, 351):
                                    try:
                                        ticarialacak_varlik_orani = round(((float(
                                            lists1[5][m].replace('.', '').replace(',', '.')[:-3]) / (float(
                                            lists1[132][m].replace('.', '').replace(',', '.')[:-3]))) * 100), 2)

                                    except ZeroDivisionError:
                                        ticarialacak_varlik_orani = 0
                                    lists[m].extend([str('%') + str(ticarialacak_varlik_orani)])
                    ...
                    for k in range(3, 13, 3):
                        with open(str(j)+' sanai '+str(k)+'.csv', 'r',encoding='utf-8-sig') as reader_file1:
                            ...
                            for m in range(1, 351):
                                try:
                                    ticarialacak_varlik_orani = round(((float(lists1[23][m].replace('.','').replace(',', '.')[:-3]) / (float(lists1[132][m].replace('.','').replace(',', '.')[:-3])))*100), 2)
